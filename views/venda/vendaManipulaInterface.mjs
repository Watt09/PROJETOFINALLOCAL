...
async function obterPrecoUnitario(produtoId) {
  try {
    ...
    console.log('Preço unitário obtido:', preco); // Log para depuração
    return preco;
  } catch (error) {
...
function preencherDataAutomatica() {
  const dataAtual = new Date(); // Obtém a data atual no horário local
  const ano = dataAtual.getFullYear();
  const mes = String(dataAtual.getMonth() + 1).padStart(2, '0'); // Meses começam em 0
  const dia = String(dataAtual.getDate()).padStart(2, '0');
  const dataFormatada = `${ano}-${mes}-${dia}`;
  document.getElementById('dataVenda').value = dataFormatada; // Define o valor no campo data
}
...
function formatarData(dataISO) {
  const data = new Date(dataISO); // Garante que a data seja tratada corretamente
